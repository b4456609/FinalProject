...
public class PicEditArea extends JPanel implements ActionListener {
	...
	public class ColorPanel extends JPanel implements MouseListener,ActionListener{//調色盤class
		...
		public void actionPerformed( ActionEvent e ){
			if(e.getSource() == left){
				center.G_color_left = JColorChooser.showDialog( PicEditArea.this, "請選擇邊線顏色", center.G_color_left );
				center.repaint();
			}
			else if(e.getSource() == right){
				center.G_color_right = JColorChooser.showDialog( PicEditArea.this, "請選擇邊線顏色", center.G_color_right );
				center.repaint();
			}
			else{
				jDialog.dispose();
		...
		public void mousePressed( MouseEvent e ){
			...
			if(e.getX()>=5 && e.getX()<=20 && e.getY()>=5 && e.getY()<=20){
				...
				color_border = JColorChooser.showDialog( PicEditArea.this, "請選擇邊線顏色", (Color)color_border );
				...
				color_inside = JColorChooser.showDialog( PicEditArea.this, "請選擇填充顏色", (Color)color_inside );
			...
			if(choose==28){//填充無顏色
				...
				FontColor = JColorChooser.showDialog( PicEditArea.this, "請選擇一個顏色當文字顏色", FontColor );//讓使用者選擇顏色
	...
	public class DrawPanel extends JPanel implements MouseListener, MouseMotionListener, ItemListener, ActionListener, ChangeListener{//中央畫布
		...
		private BufferedImage bufImg_data[]= null;//記錄所有畫出圖面，索引值越大越新，最大為最新
		...
		private JTextField textField_font = new JTextField("Fixedsys",16), textField_word = new JTextField(filesourse,16);
		...
		public void openfile(String filename){//開啟舊檔
            ...
            try {  
                ...
                bufImg_data[count] = javax.imageio.ImageIO.read(file);  
            } catch (IOException e) {  
            	e.printStackTrace();  
		...
		public void save() throws IOException{
			/*File file = new File(filename);  
			ImageIO.write( bufImg_data[count], "jpg", file);	
			*/
			
			 //new File("testImg").mkdir();
             File file=new File(filesourse);
             javax.imageio.ImageIO.write( bufImg_data[count], "jpg", file);
	...
	public void setPicture(String filesourse){
